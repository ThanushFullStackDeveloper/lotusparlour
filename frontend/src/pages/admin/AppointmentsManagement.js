...
const AppointmentsManagement = () => {
  ...
  const fetchMeta = async () => {
    const [s, st] = await Promise.all([getServices(), getStaff()]);
    setServices(s.data);
  ...
  // 🔥 Validation
  const validateForm = () => {
    if (!formData.customer_name) return "Name required";
    if (!formData.customer_phone) return "Phone required";
    if (!formData.customer_email) return "Email required";
    if (!formData.service_id) return "Select service";
    if (!formData.appointment_date) return "Select date";
    if (!formData.appointment_time) return "Select time";

    // prevent past date
    const now = new Date();
    const selected = new Date(`${formData.appointment_date} ${formData.appointment_time}`);
    if (selected < now) return "Cannot select past time";
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center">
          <div className="bg-white p-5 rounded w-96 space-y-3">
            ...
            <h2 className="font-bold">Create Appointment</h2>
